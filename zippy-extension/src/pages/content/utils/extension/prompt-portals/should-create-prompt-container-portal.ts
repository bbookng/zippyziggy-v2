...
export const shouldCreatePromptContainerPortal = (targetElement: Element): boolean => {
  ...
  const isInitialEntry = targetElement.className.includes(
    'flex flex-col text-sm dark:bg-gray-800 h-full'
  );
  // 두 번째 조건: new chat 버튼을 클릭했을 경우
  const isNewChatButtonClicked = targetElement.className.includes(
    'relative h-full w-full transition-width overflow-auto flex-1'
  );
